...
int send_multicast(void *self, const Message *msg)
{
    ...
    {
        if (dst != src)
            write(io->ppfd[src][dst][FD_WRITE], msg, sizeof(MessageHeader) + msg->s_header.s_payload_len);
    }
    return 0;
}
...
int receive(void *self, local_id src, Message *msg)
{
    ...
    while (read(io->ppfd[src][dst][FD_READ], msg, sizeof(MessageHeader)) < 0 && errno == EAGAIN)
    {
        nanosleep((const struct timespec[]){{0, timeout}}, NULL);
    }
    if (msg->s_header.s_payload_len > 0)
    {
        read(io->ppfd[src][dst][FD_READ], msg->s_payload, msg->s_header.s_payload_len);
    }

    io->last_received = src;
    incr_lamport_time(io, msg->s_header.s_local_time);
    return 0;
}
...
int receive_any(void *self, Message *msg) 
{
    ...
    {
        if (src != dst){
            if (read(io->ppfd[src][dst][FD_READ], msg, sizeof(MessageHeader)) > 0 || errno != EAGAIN)
            {
                if (msg->s_header.s_payload_len > 0)
                {
                    read(io->ppfd[src][dst][FD_READ], msg->s_payload, msg->s_header.s_payload_len);
...
Message build_msg(IO *io, size_t buff_len, MessageType type)
{
    ...
    msg.s_header = (MessageHeader)
    {
        ...
        .s_type = type,
        .s_local_time = get_lamport_time(io)
    };
    return msg;
}
